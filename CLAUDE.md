...
- **`AI_BACKENDS`** (`core/config.py`) — AI バックエンドプリセット辞書。`type` フィールドが `cli`/`api_anthropic`/`api_openai`/`api_antigravity`/`api_qwen`/`api_cohere`/`api_spark`/`api_ernie` のどれかを指定。
...
- モデルオーバーライド: `ANTHROPIC_MODEL`、`OPENAI_MODEL`、`ANTIGRAVITY_MODEL`、`DEEPSEEK_MODEL`、`QWEN_MODEL` など各 API バックエンドのデフォルトモデルを変更可能
